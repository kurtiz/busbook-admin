...
<head>
    ...
    <title>Tickets | BusBook cPanel</title>
                    ...
                    <div class="row align-items-end">
                        <div class="col-lg-8">
                            <div class="page-header-title">
                                ...
                                <div class="d-inline">
                                    <h5>Tickets</h5>
                                    <span>All tickets in your inventory</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4">
                            <nav class="breadcrumb-container" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a loading="true" href="<?= base_url(); ?>/tickets">Tickets</a>
                                    </li>
                                </ol>
                            </nav>
                        </div>
                                ...
                                <div class="col-md-12 table-responsive">
                                    <table id="ticket_data_table" class="ticket_table nowrap table">
                                        <?php if (is_array($tickets)): ?>
                                            ...
                                            <tr id="ticket<?= $tickets[$i]['ticket_id'] ?>">
                                                <td id="name<?= $tickets[$i]['ticket_id'] ?>"><?= strtoupper($tickets[$i]['ticket_id']) ?></td>
                                                <td><?= $destinations[$i] ?></td>
                                                <td><?= $drivers[$i] ?></td>
                                                <td><?= $buses[$i] ?></td>
                                                <td><?= $tickets[$i]["ticket_cost"] ?></td>
                                                <td><?= $tickets[$i]["departure_date"]. " ". $tickets[$i]["departure_time"] ?></td>
                                                <td>
                                                    <div class="table-actions">
                                                        <a href="<?= base_url(); ?>/tickets/view/<?= $tickets[$i]['ticket_id'] ?>"><i
                                                                    data-toggle="tooltip" data-placement="top"
                                                                    title="View" onclick="loading_overlay(1)"
                                                                    class="ik ik-eye text-blue"></i></a>
                                                        <a href="javascript:edit('<?= $tickets[$i]['ticket_id'] ?>')"><i
                                                                    data-toggle="tooltip" data-placement="top"
                                                                    title="Edit"
                                                                    class="ik ik-edit-2 text-green"></i></a>
                                                        <a href="javascript:delete_ticket('<?= $tickets[$i]['ticket_id'] ?>')"><i
                                                                    data-toggle="tooltip" data-placement="top"
                                                                    title="Delete"
...
<script>
    ...
    function edit(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                window.location.assign(domain + "/tickets/edit/" + id)
    ...
    function delete_ticket(id) {
        ...
        if (typeof (id) === 'object' && id.length > 0) {
            ...
            //  Shows Pop Up to confirm edit event
            Swal.fire({
                title: 'Are you sure you want to delete these tickets?',
                showDenyButton: true,
                showCancelButton: true,
                confirmButtonText: `Yes`,
                denyButtonText: `No`,
                ...
                if (result.isConfirmed) {
                    ...
                    for (let i = 0; i < id.length; i++) {
                        ...
                        url = (domain + "/tickets/delete/" + id[i].replace("ticket", ""));
