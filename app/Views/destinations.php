...
<head>
    ...
    <title>Destinations | BusBook cPanel</title>
                            ...
                            <nav class="breadcrumb-container" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a loading="true"
                                                                   href="<?= base_url(); ?>/destinations">Destinations</a>
                                    </li>
                                </ol>
                            </nav>
                        </div>
                        ...
                        <div class="card table-card">
                            ...
                            <div class="card-body">
                                <div class="col-md-12 table-responsive">
                                    <table id="destination_data_table" class="destination_table nowrap table">
                                        <?php if (is_array($destinations)): ?>
                                            ...
                                            <tr id="destination<?= $row['destination_id'] ?>">
                                                <td id="name<?= $row['destination_id'] ?>"><?= $row['destination'] ?></td>
                                                <td>
                                                    <div class="table-actions">
                                                        <a href="<?= base_url(); ?>/destinations/view/<?= $row['destination_id'] ?>"><i
                                                                    data-toggle="tooltip" data-placement="top"
                                                                    title="View" onclick="loading_overlay(1)"
                                                                    class="ik ik-eye text-blue"></i></a>
                                                        <a href="javascript:edit('<?= $row['destination_id'] ?>')"><i
                                                                    data-toggle="tooltip" data-placement="top"
                                                                    title="Edit"
                                                                    class="ik ik-edit-2 text-green"></i></a>
                                                        <a href="javascript:delete_destination('<?= $row['destination_id'] ?>')"><i
                                                                    data-toggle="tooltip" data-placement="top"
                                                                    title="Delete"
...
<link rel="stylesheet" href="<?= base_url(); ?>/public/plugins/datatables/css/buttons.dataTables.min.css">
<script>
    let table = $('.destination_table').DataTable({
        responsive: true,
        dom: 'Bflrtip',
        buttons: [
            // 'copy', 'csv', 'excel', 'pdf',
    ...
    function edit(id) {
        ...
        Swal.fire({
            title: 'Are you sure you want to edit this destination?',
            imageHeight: 100,
            text: name,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
                ...
                window.location.assign(domain + "/destinations/edit/" + id)
    ...
    /**
     * sends a post request to delete a destination for the list
     * @param {string | int | array | any} id the id of the destination to be deleted
     */
    function delete_destination(id) {
        ...
        if (typeof (id) === 'object' && id.length > 0) {
            ...
            //  Shows Pop Up to confirm edit event
            Swal.fire({
                title: 'Are you sure you want to delete these destinations?',
                showDenyButton: true,
                showCancelButton: true,
                confirmButtonText: `Yes`,
                denyButtonText: `No`,
                ...
                //  when confirm button (yes) is clicked
                if (result.isConfirmed) {
                    // post request to delete destination
                    let success = [];
                    let error = [];
                    for (let i = 0; i < id.length; i++) {
                        let name = $("#name" + id[i].replace("destination", "")).text();
                        url = (domain + "/destinations/delete/" + id[i].replace("destination", ""));
                        $.post(url, function (data) {
                            ...
                            if (data.msg === "success") {
                                ...
                                let table = $("#destination_data_table");
                                let data_table = table.dataTable();
                                let index = findProductRow(table, id[i].replace("destination", ""));
            ...
            let name = $("#name" + id).text();
            url = (domain + "/destinations/delete/" + id);
     ...
     * @returns {number} returns the index of the row of the destination to be deleted
     */
    function findProductRow(element, id) {
        ...
        for (i = 0; i < table.length; i++) {
            ...
            if (table[i].DT_RowId === "destination" + id) {
                index = i;
                break;
            }
        }
    ...
    $(document).on("keydown", function (e) {
        if (e.which === 46) {
            ...
            // console.log(ids[0].replace("destination",""))
            delete_destination(ids)
        }
    });
