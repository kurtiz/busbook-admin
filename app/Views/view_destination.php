...
<head>
    ...
    <title>View Destination | BusBook cPanel</title>
        ...
        <div class="main-content">
            <div class="container-fluid">
                            ...
                            <nav class="breadcrumb-container" aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a loading="true"
                                                                   href="<?= base_url(); ?>/destinations">Destinations</a></li>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header"><h3>View Destination</h3></div>
                            <div class="card-body">
                               <form action="" >
                                <div class="form-group">
                                    <label for="bus_model">Destination</label>
                                    <input type="text" class="form-control" value="<?= $destination->destination ?>" disabled
                                           required id="bus_model" name="bus_model" placeholder="eg. Tema">
                                </div>
                                <a href="<?= base_url() ?>/destinations/edit/<?= $destination->destination_id ?>" class="btn btn-success mr-2">Edit</a>
                                <button type="button" id="deleteDestination" class="btn btn-danger mr-2">Delete</button>
                               </form>
                            </div>
                        </div>
                    </div>
    ...
    $("#deleteDestination").on("click", function(){
        Swal.fire({
            ...
            if (result.isConfirmed) { //  when confirm button (yes) is clicked
                $.post("<?=base_url()?>/destinations/delete/<?=$destination->destination_id?>", function (data) {
                    try {
                    ...
                    if (data.msg === "success") {
                        Swal.fire('Destination successfully deleted!', '', 'success')
                        setTimeout(() => {
                            location.href = "<?=base_url()?>/destinations"
                        }, 1500)
                    } else {
                        $.toast({
                            text: 'An error occurred while deleting this bus',
                            showHideTransition: 'fade',
                            icon: 'error',
                            position: "top-right",
                            bgColor: '#f5365c',
